...
def get_choice_text(columnName, field):
    """
    显示选项字段的中文信息
    :param columnName: 希望页面显示的表头
    :param field: 字段名称
    :return:
    """
    def inner(self, obj=None, is_header=None, *args, **kwargs):
        return columnName if is_header else getattr(obj, "get_%s_display" % field)()
    return inner
...
def get_datetime_text(title, field, time_format='%Y-%m-%d'):
    ...
    """
    def inner(self, obj=None, is_header=None, *args, **kwargs):
        return title if is_header else getattr(obj, field).strftime(time_format)
    return inner
...
def get_m2m_text(title, field):
    ...
    """
    def inner(self, obj=None, is_header=None, *args, **kwargs):
        return title if is_header else ','.join([str(row) for row in getattr(obj, field).all()])
    return inner
...
class SearchGroupRow(object):
    ...
    def __iter__(self):
        ...
        if not origin_value_list:
            yield "<a class='active btn' href='?%s'>全部</a>" % total_query_dict.urlencode()
        else:
...
class StarkHandler(object):
    ...
    def display_checkbox(self, obj=None, is_header=None, *args, **kwargs):
        ...
        :return:
        """
        return "选择" if is_header else mark_safe('<input type="checkbox" name="pk" value="%s" />' % obj.pk)
    ...
    def display_edit(self, obj=None, is_header=None, *args, **kwargs):
        ...
        :return:
        """
        return "编辑" if is_header else mark_safe('<a href="%s">编辑</a>' % self.reverse_change_url(pk=obj.pk))

    def display_del(self, obj=None, is_header=None, *args, **kwargs):
        return "删除" if is_header else mark_safe('<a href="%s">删除</a>' % self.reverse_delete_url(pk=obj.pk))
